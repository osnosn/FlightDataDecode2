...
def main():
    ...
    fzip.close()
    '''
    print('   Note: word=16bit, byte=8bit ')
    ...
    for byte in getbyte(buf):
        ...
        if mark<0 and word == 0x247:
            mark=ii
            print('------->Mark sync1.at x{:<5X} word + {:01d}byte'.format(word_cnt,ii))
        #if ii==mark: #两字节对齐
        if True: #不对齐
            if word == 0x247:
                print('==>Found sync1.247,at x{:<5X} word + {:01d}byte, len:x{:<5X}'.format(word_cnt,ii,word_cnt-word_cnt2))
                word_cnt2=word_cnt
            elif word == 0x5B8:
                print('==>Found sync2.5B8,at x{:<5X} word + {:01d}byte, len:x{:<5X}'.format(word_cnt,ii,word_cnt-word_cnt2))
                word_cnt2=word_cnt
            elif word == 0xA47:
                print('==>Found sync3.A47,at x{:<5X} word + {:01d}byte, len:x{:<5X}'.format(word_cnt,ii,word_cnt-word_cnt2))
                word_cnt2=word_cnt
            elif word == 0xDB8:
                print('==>Found sync4.DB8,at x{:<5X} word + {:01d}byte, len:x{:<5X}'.format(word_cnt,ii,word_cnt-word_cnt2))
                word_cnt2=word_cnt
