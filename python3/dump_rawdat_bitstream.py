...
def main(move='>>', order='high'):
    ...
    for bit in getbit(buf, order):
        ...
        if mark<0 and word == 0x247:
            mark=ii
            print('------->Mark sync1.at x{:<5X} word + {:02d}bit'.format(word_cnt,ii))
            #print('==>Mark,sync1,at x%X word(12bit) + %d bit'%(word_cnt,ii,))
        if ii==mark: #12bit 对齐
        #if True: #12bit 不对齐
            if word == 0x247:
                print('==>Found sync1.247,at x{:<5X} word + {:02d}bit, len:x{:<5X}'.format(word_cnt,ii,word_cnt-word_cnt2))
                #print('==>Found sync1.247,at x%X word(12bit)+%dbit,len:x%X'%(word_cnt,ii,word_cnt-word_cnt2))
                word_cnt2=word_cnt
            elif word == 0x5B8:
                print('==>Found sync2.5B8,at x{:<5X} word + {:02d}bit, len:x{:<5X}'.format(word_cnt,ii,word_cnt-word_cnt2))
                #print('==>Found sync2.5B8,at x%X word(12bit)+%dbit,len:x%X'%(word_cnt,ii,word_cnt-word_cnt2))
                word_cnt2=word_cnt
            elif word == 0xA47:
                print('==>Found sync3.A47,at x{:<5X} word + {:02d}bit, len:x{:<5X}'.format(word_cnt,ii,word_cnt-word_cnt2))
